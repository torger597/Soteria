	...
	if(candidates.len)
		...
		for (var/obj/effect/landmark/L in world)
			if(L.name == "voxstart")
				...
				new_vox << "\blue You are a Vox Primalis, fresh out of the Shoal. Your ship has arrived at the Andromeda system hosting the NSS Hypatia... or was it the Luna? NSV? Utopia? Nobody is really sure, but everyong is raring to start pillaging! Your current goal is: \red<B> [input]</B>"
...
/datum/admins/proc/create_vox_raider(obj/spawn_location, leader_chosen = 0)
	...
	new_vox.mutations |= NOCLONE //Stops the ship crew from messing around with their DNA.
