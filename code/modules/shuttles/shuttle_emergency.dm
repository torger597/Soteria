...
/datum/shuttle/ferry/emergency/move(var/area/origin,var/area/destination)
	...
	if (origin == area_station)	//leaving the station
		...
		if (emergency_shuttle.evac)
			captain_announce("The Escape Pods have launched. Estimate [round(emergency_shuttle.estimate_arrival_time()/60,1)] minutes until the pods arrive at the NMV-LC .")
		else
			captain_announce("The shift will end in [round(emergency_shuttle.estimate_arrival_time()/60,1)] minutes.")
