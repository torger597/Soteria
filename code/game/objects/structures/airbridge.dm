	...
	proc/establish_bridge()
		...
		spawn(0)
			...
			for(var/turf/T in path)
				var/list/added = list()
				var/turf/curr = new/turf/simulated/shuttle/floor { oxygen = 0 nitrogen = 0 } (T)
				...
				for(var/i = 1, i <= tunnel_width, i++)
					...
					if(i == tunnel_width)
						curr = new/turf/simulated/wall/r_wall ( get_steps(T, turn(path[T], 90),i) )
						maintaining_turfs.Add(curr)
						added.Add(curr)
						...
						curr = new/turf/simulated/shuttle/floor { oxygen = 0 nitrogen = 0 } ( get_steps(T, turn(path[T], 90),i) )
						maintaining_turfs.Add(curr)
						added.Add(curr)
	...
	Topic(href, href_list)
		...
		if (href_list["create"])
			...
			if (src.establish_bridge())
				message_admins("[usr] extended the airbridge at [usr.loc.loc] ([showCoords(usr.x, usr.y, usr.z)])")
			...
			if (src.remove_bridge())
				message_admins("[usr] retracted the airbridge at [usr.loc.loc] ([showCoords(usr.x, usr.y, usr.z)])")
			...
			if (!(src.allowed(usr)))
				...
				return
			if (src.pressurize())
				message_admins("[usr] pressurized the airbridge at [usr.loc.loc] ([showCoords(usr.x, usr.y, usr.z)])")
